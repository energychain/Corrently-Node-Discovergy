...
service(function(db,kv) {
  ...
  var fillHistoryDb=function(historic_days,max_ts) {
    ...
    function consumer(vector,to) {
      return new Promise(function(resolve,reject) {
        from=new Date(to).getTime()-(vector.timestep*((itterations*2)*vector.contextstep));
          var request_options={};
          request_options={
              url:"https://"+process.env.DISCOVERGY_USERNAME+":"+process.env.DISCOVERGY_PASSWORD+"@api.discovergy.com/public/v1/readings?meterId="+process.env.DISCOVERGY_METERID+"&resolution="+vector.resolution+"&from="+new Date(from).getTime()+"&to="+new Date(to).getTime()
          }
    ...
    fillHistory();
      ...
      db.get("performance").then(function(doc) {
          history=doc.values;
          ...
          fillHistoryDb(360,0).then(function(x) {
            ...
            doc.values=history;
            db.put(doc)
                  .then(function(c) { console.log("FINISHED");  });
          });
      });
});
